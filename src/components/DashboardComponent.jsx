...
/// TODO remove and replace with tokens of wallet owner fetch
import { NFTS } from "./ExampleArray";

function Dashboard({ isLoading, setIsLoading }) {
  const [nfts, setNfts] = useState([]);
  const [selectedArray, setSelectedArray] = useState([]);
  // const { balanceOf, reload } = useNftContract();

  useEffect(() => {
    setSelectedArray(new Array(NFTS.length));

    /// TODO replace NFTS with tokens of wallet owner fetch
    fetchSassyInfo(NFTS).then((sassies) => setNfts(sassies));
  }, []);
  ...
  async function fetchSassyInfo(nfts) {
    ...
    return sassyImgs.map(({ id, imgUrl }, idx) => {
      return {
        id,
        imgUrl,
        lodgeStatus: sassyLodging[idx].lodgeStatus,
      };
    });
  }
  ...
  async function fetchLodgeState(nfts) {
    /// TODO replace with actual lodging status fetch
    const lodgingPromises = nfts.map((id) => {
      return {
        id,
        lodgeStatus: id % 2 == 0,
      };
    });
    return lodgingPromises;
  }
            ...
            <div className="row m-5 p-5">
              ...
              <div className="tab-content" id="nav-tabContent">
                ...
                >
                  <div className="row">
                    {nfts &&
                      nfts.map(({ id, lodgeStatus, imgUrl }, idx) => {
                        const selected =
                            ...
                            <div className="card border-light">
                              ...
                              <div className="card-body">
                                ...
                                <div className="row">
                                  <p
                                    className={
                                      lodgeStatus ? "mont-semi2" : "d-none"
                                    }
                                  >
                                    {lodgeStatus ? "Sassy lodged" : ""}
                                  </p>
                                  ...
                                  <button
                                    ...
                                    style={
                                      lodgeStatus ? { display: "none" } : {}
                                    }
                                    onClick={() => handleSelect(idx)}
                                    disabled={lodgeStatus ? true : false}
                                  >
                                    {lodgeStatus ? "" : "Select"}
                                  </button>
                                </div>
